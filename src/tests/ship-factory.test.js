...
test("check if ship factory properly creates ship body array of given length 6", () =>
{
  ...
  expect(ship.getShipBody().length).toEqual(4);

  expect(ship2.getShipBody().length).toEqual(3);

  expect(ship3.getShipBody().length).toEqual(2);
});
...
test("check if ship properly register hits", () =>
{
  ...
  expect(ship.getShipBody()[1]).toEqual([2, 0]);
});
...
test("check if calculating ship area works properly with horizontal placement", () =>
{
  ...
  expect(ship.calculateShipArea([1, 1])).toEqual([
    [1, 1],
    [2, 1],
    [3, 1],
    [4, 1],
...
test("check if shipFactory correctly save ship coordinates", () =>
{
  ...
  expect(ship.getShipArea()).toEqual([
    [1, 1],
    [1, 2],
    [1, 3],
    [1, 4],
...
test("check if calculate collision area works properly", () =>
{
  ...
  expect(ship.calculateCollisionArea(ship.getShipArea())).toEqual([
    [1, 0],
    [2, 1],
    [1, 1],
    [3, 1],
