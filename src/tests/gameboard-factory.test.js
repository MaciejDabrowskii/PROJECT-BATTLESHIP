...
test("check if gameboardFactory properly adds ships to gameboard - horizontally", () =>
{
  ...
  expect(typeof gameboard.getBoard()[1][1]).toEqual("object");

  expect(typeof gameboard.getBoard()[2][1]).toEqual("object");

  expect(typeof gameboard.getBoard()[1][2]).toEqual("undefined");
});
...
test("check if gameboardFactory properly adds ships to gameboard - vertically", () =>
{
  ...
  expect(typeof gameboard.getBoard()[0][0]).toEqual("object");

  expect(typeof gameboard.getBoard()[0][1]).toEqual("object");

  expect(typeof gameboard.getBoard()[1][0]).toEqual("undefined");
});
...
test("check if placed ship methotds work properly", () =>
{
  ...
  expect(gameboard.getBoard()[1][1].getShipArea()).toEqual([
    [1, 1],
    [2, 1],
    [3, 1],
    [4, 1],
...
test("check if recive attack correctly register hits ", () =>
{
  ...
  expect(gameboard.getBoard()[1][0].getShipBody()[1]).toEqual([1, 0]);

  expect(gameboard.getBoard()[1][0].getShipBody()).toEqual([
    [0, 0],
    [1, 0],
  ]);

  expect(gameboard.getBoard()[7][5].getShipBody()).toEqual([
    "notHit",
    "notHit",
    "notHit",
    "notHit",
...
test("check if reciveAttack correctly register misses ", () =>
{
  ...
  expect(gameboard.getBoard()[2][0]).toEqual("miss");

  expect(gameboard.getBoard()[0][1]).toEqual("miss");

  expect(gameboard.getBoard()[1][1]).toEqual("miss");
});
...
test("check if gameboard correctly stores misses ", () =>
{
  ...
  expect(gameboard.getFieldStatus().missedAttacks).toEqual([
    [2, 0],
    [3, 0],
    [4, 0],
    [5, 0],
...
test("check if isColliding works", () =>
{
  ...
  expect(gameboard.getFieldStatus().antiCollision.length).toEqual(6);
});
...
test("check if gamebord correctly register colison coordinates ", () =>
{
  const gameboard = gameboardFactory();
  gameboard.placeShip("destroyer", [1, 1]);
  expect(gameboard.getFieldStatus().antiCollision.length).toEqual(12);
});
...
test("check if calulateColision not returning coords outside gameboard < 0 ", () =>
{
  ...
  expect(gameboard.getFieldStatus().antiCollision).toEqual([
    [0, 0],
    [1, 0],
    [0, 1],
    [1, 1],
...
test("check if placement of all ships and anticolission works ", () =>
{
  ...
  expect(_.compact(_.flattenDeep(gameboard.getBoard())).length).toEqual(17);

  expect(gameboard.getFieldStatus().antiCollision.length).toEqual(67);
});
...
test("check if placing all ships and geting hit to all of them and isFleetDestroyed works correctly", () =>
{
  ...
  expect(gameboard.getShips().carrier.getShipBody()).toEqual([
    [0, 0],
    [1, 0],
    [2, 0],
    [3, 0],
  ...
  expect(gameboard.getShips().carrier.isSunk()).toEqual(true);

  expect(gameboard.getShips().battleship.getShipBody()).toEqual([
    [0, 2],
    [1, 2],
    [2, 2],
    [3, 2],
  ...
  expect(gameboard.getShips().battleship.isSunk()).toEqual(true);

  expect(gameboard.getShips().crusier.getShipBody()).toEqual([
    [0, 4],
    [1, 4],
    [2, 4],
  ]);

  expect(gameboard.getShips().crusier.isSunk()).toEqual(true);

  expect(gameboard.getShips().submarine.getShipBody()).toEqual([
    [0, 6],
    [1, 6],
    [2, 6],
  ]);

  expect(gameboard.getShips().submarine.isSunk()).toEqual(true);

  expect(gameboard.getShips().destroyer.getShipBody()).toEqual([
    [0, 8],
    [1, 8],
  ]);

  expect(gameboard.getShips().destroyer.isSunk()).toEqual(true);
...
test("check if function checkForInvalidCoords prevents from placing ships with coords that force ship to sticking outside gameboard", () =>
{
  ...
  expect(_.compact(_.flattenDeep(gameboard.getBoard())).length).toEqual(0);
});
